...
type AllowedParamsSchema = Record<string, AllowedParamType>;
...
router.get('/search', async (req, res) => {
  try {
    ...
    if (typeof req.query.tutor === 'string' && req.query.tutor.trim() !== '') {
      query['responses.tutor'] = {
        $regex: escapeRegex(req.query.tutor),
        $options: 'i',
      };
    }
    ...
    if (
      ...
      query['responses.subject'] = {
        $regex: escapeRegex(req.query.subject),
        $options: 'i',
      };
    }

    if (req.query.paper) {
      const papers = Array.isArray(req.query.paper)
        ? req.query.paper
        : [req.query.paper];
    ...
    const processedReviews: IProcessedReview[] = reviews.map((review) => {
      ...
      const isOld =
        new Date().getTime() - new Date(reviewObj.submittedAt).getTime() >
        3 * 365 * 24 * 60 * 60 * 1000;
      return {
        ...reviewObj,
        _id: crypto
          .createHash('sha256')
          .update(reviewObj._id.toString())
          .digest('hex'),
        isOld,
      };
    });
    ...
    const groupedReviews = processedReviews.reduce(
      (acc, review) => {
    ...
    const requestedColleges = Array.isArray(req.query.college)
      ...
      : typeof req.query.college === 'string'
        ? [req.query.college]
        : [];
    ...
    const finalReviews = Object.values(groupedReviews)
      .filter((group) => {
        ...
        const recentCollegeCount = group.reduce(
          (acc, review) => {
            if (!review.isOld) {
        ...
        return requestedColleges.some(
          (college) => (recentCollegeCount[college] || 0) >= 3,
        );
      })
      .flatMap((group) => {
        const recentCollegeCount = group.reduce(
