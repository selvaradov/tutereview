...
const SearchPage: React.FC = () => {
  ...
  const subjectOptions: Option[] = Object.keys(papersBySubject).map(
    (subjectName) => ({
      value: subjectName,
      label: subjectName,
    }),
  ...
  const memoizedCollegeOptions = useMemo(
    () =>
      searchParams.college
        .map((value) => colleges.find((c) => c.value === value))
  ...
  const memoizedPaperOptions = useMemo(
    () =>
      searchParams.paper
        .map((value) => paperOptions.find((p) => p.value === value))
        .filter(Boolean) as Option[],
          ...
          <Col md={6} className="mb-3">
            <Form.Group controlId="paper">
              ...
              <Select<Option, true>
                ...
                placeholder={
                  selectedSubject ? 'Select papers' : 'Choose subject first'
                }
                isMulti
                inputId="paper"
              />
      ...
      <div id="results">
        {collegeFilterApplied && (
          <Alert variant="info" className="mb-3">
            Note: When filtering by college, only tutor-paper combinations with
            at least three recent submissions from the college(s) in question
            are shown.
          </Alert>
        )}
        {areSearchParamsEmpty(latestSearchParams.current) ? (
          ...
          <>
            <p>
              You can now see how other students found their tutorials,
              including how helpful the tutor's answers to their questions were,
              whether the tutor had looked at their work before the tutorial,
              how tutorials were structured, and the type of feedback they
              received.
            </p>
            {Object.entries(groupedReviews).map(
              ([key, { reviews, showFullResults, colleges }]) => {
